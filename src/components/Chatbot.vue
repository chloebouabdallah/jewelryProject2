<template>
  <div>
    <button @click="toggleChatbot" class="fixed bottom-6 right-6 z-50 bg-amber-600 hover:bg-amber-700 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110">
      ...
      <div v-if="isOpen" class="fixed top-0 right-0 w-full sm:w-96 h-full bg-white shadow-2xl z-50 flex flex-col">
        <div class="bg-gradient-to-r from-amber-600 to-amber-500 text-white p-4 flex justify-between items-center">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs opacity-90">Online | 24/7</p>
            </div>
          </div>
          <button @click="toggleChatbot" class="text-white hover:text-amber-200 transition"><i class="fas fa-times text-xl"></i></button>
        </div>
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-3 bg-amber-50/30">
          <div v-for="(msg, index) in messages" :key="index" class="flex" :class="msg.sender === 'user' ? 'justify-end' : 'justify-start'">
            <div :class="msg.sender === 'user' ? 'bg-amber-600 text-white rounded-br-sm' : 'bg-white border border-amber-200 text-stone-700 rounded-bl-sm'" class="max-w-[80%] p-3 rounded-2xl">
          ...
          <div v-if="isTyping" class="flex justify-start">
            <div class="bg-white border border-amber-200 p-3 rounded-2xl rounded-bl-sm">
              <div class="flex gap-1"><span class="w-2 h-2 bg-amber-400 rounded-full animate-bounce" style="animation-delay:0ms"></span><span class="w-2 h-2 bg-amber-400 rounded-full animate-bounce" style="animation-delay:150ms"></span><span class="w-2 h-2 bg-amber-400 rounded-full animate-bounce" style="animation-delay:300ms"></span></div>
            </div>
          </div>
        </div>

        <div class="px-4 py-2 border-t border-amber-100 overflow-x-auto">
          <div class="flex flex-wrap gap-2">
            <button v-for="reply in quickReplies" :key="reply.text" @click="sendQuickReply(reply.text)" class="text-xs bg-amber-100 text-amber-700 px-3 py-1.5 rounded-full hover:bg-amber-200 transition whitespace-nowrap">
        ...
        <div class="p-4 border-t border-amber-100 bg-white">
          <div class="flex gap-2">
            <input v-model="userInput" @keypress.enter="sendMessage" type="text" placeholder="Ask me anything about jewelry, shipping, colors..." class="flex-1 px-4 py-2 border border-amber-200 rounded-full focus:outline-none focus:ring-2 focus:ring-amber-400 text-sm">
            <button @click="sendMessage" :disabled="!userInput.trim()" class="bg-amber-600 hover:bg-amber-700 text-white rounded-full w-10 h-10 flex items-center justify-center transition disabled:opacity-50"><i class="fas fa-paper-plane text-sm"></i></button>
          </div>
        </div>
      </div>
    </Transition>
...
function addMessage(sender, text) {
  messages.value.push({ sender, text, time: getCurrentTime() })
  scrollToBottom()
}
