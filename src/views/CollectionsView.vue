...
<template>
  <main>
    <!-- Hero -->
    <section class="pt-28 md:pt-32 pb-12 md:pb-20 px-5 bg-gradient-to-br from-[#f5e6d8] via-[#efe0d2] to-[#f5e6d8]">
      <div class="max-w-7xl mx-auto text-center">
        <span class="text-amber-700 tracking-[0.2em] text-xs md:text-sm uppercase font-semibold animate-fade-scale">
          Discover By Category
        </span>
        <h1 class="font-playfair text-3xl md:text-5xl lg:text-7xl font-light text-stone-800 mb-3 md:mb-4 animate-fade-scale">
          Shop by Collection
        </h1>
        <div class="w-16 md:w-24 h-0.5 bg-amber-500 mx-auto mb-4 md:mb-6 rounded-full animate-fade-scale"></div>
        <p class="text-stone-600 text-sm md:text-lg max-w-2xl mx-auto animate-fade-scale">
    ...
    <template v-else>
      ...
      >
        <div class="max-w-7xl mx-auto px-5 md:px-8">
          <div class="flex flex-col md:flex-row items-center gap-6 md:gap-0 group" :class="index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'">
            
            <!-- Image -->
            <div class="md:w-1/2 relative group/image fade-on-scroll fade-right">
              <div class="relative w-full max-w-sm md:max-w-md mx-auto" :class="index % 2 === 0 ? 'md:mr-auto md:ml-0' : 'md:ml-auto md:mr-0'">
                <div
            ...
            >
              <div class="max-w-md py-6 md:py-12" :class="index % 2 === 0 ? 'text-left' : 'text-right'">
                <div class="flex items-center gap-2 mb-3 md:mb-4" :class="index % 2 === 0 ? 'justify-start' : 'justify-end'">
                  ...
                  <span class="text-amber-700 tracking-[0.2em] text-[10px] md:text-xs uppercase font-semibold">
                    {{ getCategoryLabel(category) }}
                  </span>
                </div>
                
                <h2 class="font-playfair text-2xl md:text-4xl lg:text-5xl font-light text-stone-800 leading-tight mb-3 md:mb-5">
                  {{ getCategoryTitle(category) }}
                  <span class="font-bold text-amber-800/90">{{ getCategorySuffix(category) }}</span>
                </h2>
                
                <p class="text-stone-700 text-sm md:text-base lg:text-lg leading-relaxed mb-4 md:mb-6">
...
function getCategoryImage(category) {
  ...
  return mediaAPI.getImageUrl(category.image)
}
